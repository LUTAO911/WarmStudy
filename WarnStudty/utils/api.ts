...
export function searchPsychologyKnowledge(
  ...
  return request(
    `/api/agent/psychology/knowledge?q=${encodeURIComponent(query)}&user_type=${userType}&n=${n}`,
    undefined,
    "GET",
  );
}
...
export function getPsychologyCategories(userType: string = "student"): Promise<{
  ...
  return request(
    `/api/agent/psychology/categories?user_type=${userType}`,
    undefined,
    "GET",
  );
}
...
export function getCheckinHistory(
  ...
  return request(
    `/api/student/checkin/${userId}?days=${days}`,
    undefined,
    "GET",
  );
}
 ...
 */
export function getChildCheckins(
  childId: string,
  days: number = 7,
): Promise<any> {
  return request(
    `/api/parent/child/${childId}/checkins?days=${days}`,
    undefined,
    "GET",
  );
}
...
export function getDailyAdvice(
  childId: string,
): Promise<{ advice: string; focus: string }> {
...
export function getChildrenProfiles(childIds: string[]): Promise<{
  ...
  return request("/api/parent/children/profiles", {
...
export function bindChild(parentId: string, childId: string): Promise<any> {
  return request("/api/parent/child/bind", {
    parent_id: parentId,
    child_id: childId,
  });
}
 ...
 * 用扫码得到的 token 绑定家长
 */
export function bindParentByToken(
  token: string,
  childId: string,
): Promise<{ success: boolean; error?: string; parent_name?: string }> {
...
/** 获取孩子测评报告列表 */
export function getChildPsychReports(
  childId: string,
  limit = 5,
): Promise<{
  success: boolean;
...
/** 获取预警列表 */
export function getParentAlerts(
  parentId: string,
  limit = 20,
  offset = 0,
): Promise<{
