...
class Router:
    ...
    def decide(self, message: str, session_id: str) -> RouteDecision:
        """
        智能路由判断：先查 Redis 缓存，未命中则用 Qwen-Turbo 判断，不可用时关键词兜底
        """
        # 1. 查缓存
        ...
        try:
            ...
            if not rag and not tools and not skills:
                decision = RouteDecision(False, False, False, 0.5, "fallback")
            else:
                decision = RouteDecision(rag, tools, skills, conf, "qwen_turbo")
        except Exception:
            # 关键词兜底
