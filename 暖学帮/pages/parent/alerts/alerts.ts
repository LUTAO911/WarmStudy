...
const alertTypeMap: Record<string, { icon: string; color: string; label: string; bg: string }> = {
  ...
  test_concerning: { icon: '📊', color: '#f44336', label: '测评', bg: '#ffebee' },
...
Page({
  ...
  loadAlerts(reset: boolean) {
    ...
    const parentId = wx.getStorageSync('parent_user_id') || 'parent_001';
    ...
    getParentAlerts(parentId, this.data.limit, offset)
      .then((res: any) => {
        if (res.success) {
          const alerts = reset ? res.alerts : [...this.data.alerts, ...res.alerts];
          this.setData({
            alerts,
    ...
    markAllAlertsRead(parentId)
      .then((res: any) => {
        if (res.success) {
          ...
          this.setData({ alerts });
          wx.showToast({ title: `已读 ${res.marked_count} 条`, icon: 'success' });
        }
      })
      .catch(() => {});
  },
